...
_INTERVIEW_PREP_SCHEMA_READY = False


INTERVIEW_PREP_SCHEMA_SQL = """
...
def ensure_interview_prep_schema() -> None:
    global _INTERVIEW_PREP_SCHEMA_READY

    if _INTERVIEW_PREP_SCHEMA_READY:
        return
    ...
    try:
        cur.execute(INTERVIEW_PREP_SCHEMA_SQL)
        conn.commit()
        _INTERVIEW_PREP_SCHEMA_READY = True
    finally:
        cur.close()
        conn.close()
